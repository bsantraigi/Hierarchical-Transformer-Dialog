...
def evaluate(model, args, dataset, dataset_counter, batch_size, criterion, split, method='beam', beam_size=None):
    ...
    logger.debug(
        '==> {} \tLoss: {:0.4f}\tBleu: {:0.3f}\tF1-Entity {:0.3f}\tInform {:0.3f}\tSuccesses: {:0.3f}\tCriteria: {:0.3f}\tTime taken: {:0.1f}s'.format(
            split, total_loss, score, f1_entity, matches, successes, criteria, elapsed))
...
def training(model, args, criterion, optimizer, scheduler, optuna_callback=None):
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        val_loss, val_bleu, val_f1entity, matches, successes = evaluate(model, args, val, val_counter, args.batch_size,
                                                                        criterion, 'val', 'greedy')
...
def testing(model, args, criterion, split, method):
    ...
    test_loss, test_bleu, test_f1entity, matches, successes = evaluate(model, args, data, dataset_counter,
                                                                       args.batch_size, criterion, split, method)
...
def run(args, optuna_callback=None):
    ...
    print('Total number of trainable parameters: ',
          sum(p.numel() for p in model.parameters() if p.requires_grad) / float(1000000), 'M')
