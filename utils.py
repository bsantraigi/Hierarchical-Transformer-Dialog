...
def data_gen_acts(dataset,  datatset_db, act_vecs, batch_size, i, wordtoidx):
	# print(i, len(dataset))
	max_dial_len = len(dataset[i])-1
...
def data_loader_acts(dataset, dataset_counter, datatset_db, act_vecs, batch_size, wordtoidx): 
	# return batches according to dialog len, -> all similar at once
	# do mask also for these
	prev=0
	for dial_len, val in dataset_counter.items():
	...
	#        continue
		for i in range(prev, prev+val, batch_size):
#             print(i, min(batch_size, prev+val-i))
			yield data_gen_acts(dataset, datatset_db, act_vecs, min(batch_size, prev+val-i), i, wordtoidx)
