...
def _NORMAL_masks(tgt_seq, src_seq, input_mask, utt_loc):
    # HT-Encoder
    pe_utt_loc = _get_pe_inputs(tgt_seq, src_seq, input_mask, utt_loc)
    ...
    # enc_mask = enc_mask_ct
    return pe_utt_loc, None, enc_mask_ct, dec_enc_attn_mask

def get_hier_encoder_mask(tgt_seq, src_seq, input_mask, utt_loc, type:str):
    """
    Returns all masks required for an hier-transformer seq2seq model
    ...
    # CT-Mask type
    assert type in ["hier", "cls", "full", "normal"]
    ...
    if type == "hier": # HIER: Context through final utterance
        ...
        return _NORMAL_masks(tgt_seq, src_seq, input_mask, utt_loc)
